...
def compute_RT_projection_2d_symmetry(RT_1, RT_2, class_id, handle_visibility, mesh_vertices, intrinsics, synset_names,
                                      num_rotation=20):
    '''
    :param RT_1: [4, 4]. homogeneous affine transformation
    :param RT_2: [4, 4]. homogeneous affine transformation
...
def rotate_and_crop_images(image, masks, coords, rotate_degree):
    ...
    image_rotated_cropped = cv2.resize(image_rotated_cropped, (image_width, image_height),
                                       interpolation=cv2.INTER_LINEAR)
    mask_rotated_cropped = cv2.resize(mask_rotated_cropped, (image_width, image_height),
    ...
    if coords is not None:
        ...
        coord_rotated_cropped = cv2.resize(coord_rotated_cropped, (image_width, image_height),
                                           interpolation=cv2.INTER_NEAREST)
...
def compute_2D_projection_mAP(final_results, synset_names, log_dir, meshes, intrinsics, iou_3d_thresholds=[0.1],
                              iou_pose_thres=0.1, projection_2d_thresholds=[5], use_matches_for_pose=False):
    """Compute Average Precision at a set IoU threshold (default 0.5).
    Returns:
    mAP: Mean Average Precision
    precisions: List of precisions at different class score thresholds.
    ...
    for progress, result in enumerate(final_results):
        ...
        for cls_id in range(1, num_classes):
            ...
            RT_projection_2d_overlaps = compute_RT_projection_2d_overlaps(cls_gt_class_ids, cls_gt_RTs,
                                                                          cls_gt_handle_visibility,
                                                                          cls_pred_class_ids, cls_pred_RTs,
                                                                          meshes, intrinsics, synset_names)

            projection_cls_gt_match, projection_cls_pred_match = compute_match_from_projection_2d_dist(
            ...
            projection_pred_matches_all[cls_id] = np.concatenate(
                (projection_pred_matches_all[cls_id], projection_cls_pred_match), axis=-1)
            ...
            assert projection_pred_scores_all[cls_id].shape[1] == projection_pred_matches_all[cls_id].shape[
                1], '{} vs. {}'.format(projection_pred_scores_all[cls_id].shape,
    ...
    projection_dict_pkl_path = os.path.join(log_dir,
                                            prefix + 'Projection_2D_AP_{}-{}pixel.pkl'.format(projection_thres_list[0],
                                                                                              projection_thres_list[
                                                                                                  -1]))
    pose_dict = {}
    ...
    output_path = os.path.join(log_dir, prefix + 'Projection_2D_AP_{}-{}pixel.png'.format(projection_thres_list[0],
                                                                                          projection_thres_list[-1]))
    fig_pose.legend()
...
def compute_coords_aps(final_results, synset_names, iou_thresholds, coord_thresholds):
    ...
    for progress, result in enumerate(final_results):
        ...
        for cls_id in range(1, num_classes):
            ...
            gt_match, pred_match, overlaps, pred_indices = compute_matches(cls_gt_bboxes, cls_gt_class_ids,
                                                                           cls_gt_masks,
                                                                           cls_pred_bboxes, cls_pred_class_ids,
                                                                           cls_pred_scores,
                                                                           cls_pred_masks, 0.5)
...
def draw_detections_wogt(image, save_dir, data_name, image_id, intrinsics, synset_names, draw_rgb_coord,
                         pred_bbox, pred_class_ids, pred_mask, pred_coord, pred_RTs, pred_scores, pred_scales,
    ...
    if draw_pred:
        ...
        #             cls_id = pred_class_ids[ind]
        #
        #             degree, cm = compute_RT_degree_cm_symmetry(RT, RT, cls_id, gt_handle_visibility, synset_names)
        #             text = '{}({:.1f}, {:.1f})'.format(synset_names[cls_id], degree, cm)
        #             overlay = draw_text(overlay, pred_bbox[ind], text)
        #             cv2.addWeighted(overlay, alpha, draw_image_bbox, 1 - alpha, 0, draw_image_bbox)
...
def align(class_ids, masks, coords, depth, intrinsics, synset_names, image_path, save_path=None, if_norm=False,
          with_scale=True, verbose=False):
    ...
    for i in range(num_instances):
        ...
        try:
            ...
            # if verbose:
            if verbose == 0:  # By jiawei: modify from line3100 to line 3101
                print('Mask ID: ', i)
            ...
            message = '[ Error ] aligning instance {} in {} fails. Message: {}.'.format(synset_names[class_id],
                                                                                        image_path, str(e))
            print(message)
...
def align_ICP(class_ids, masks, depth, intrinsics, synset_names, image_path, save_path=None, if_norm=False,
              verbose=False):
    num_instances = len(class_ids)
    error_messages = ''
